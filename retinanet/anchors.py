...
class Anchors(nn.Module):
    def __init__(self, pyramid_levels=None, strides=None, sizes=None, ratios=None, scales=None):
        ...
        self.ratios = ratios or np.array([0.5, 1, 2])
        self.scales = scales or np.array([2 ** 0, 2 ** (1.0 / 3.0), 2 ** (2.0 / 3.0)])
    ...
    def forward(self, image):
        ...
        all_anchors = torch.from_numpy(all_anchors.astype(np.float32)).unsqueeze(0)
        all_anchors = all_anchors.repeat(image.shape[0], 1, 1)  # Repeat for batch size

        if torch.cuda.is_available():
            return all_anchors.cuda()
        return all_anchors
